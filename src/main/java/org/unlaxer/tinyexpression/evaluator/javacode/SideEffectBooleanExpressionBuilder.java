...
public class SideEffectBooleanExpressionBuilder implements CodeBuilder {
	...
	public static SideEffectBooleanExpressionBuilder SINGLETON = new SideEffectBooleanExpressionBuilder();

	@Override
	public void build(SimpleJavaCodeBuilder builder, Token token) {
		
		MethodAndParameters methodAndParameters = SideEffectBooleanExpressionParser.extract(token);
		...
		BooleanBuilder.SINGLETON.build(builder, methodAndParameters.parameterTokens.get(0).filteredChildren.get(0));
