...
1. AST coverage beyond numeric binary core:
   ...
   3. `ObjectExpression` 複合式の一部は still bridge 依存（structured-text/method body/method argument の multi-root AST retry で縮小済み、`_astEvaluatorGeneratedEmbeddedBridgeUsed` で可視化可能だが declaration-heavy / nested complex slices は未完）。
