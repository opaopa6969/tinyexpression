...
let mut out=self.e38_c(state);
if out.ok {
let mut scope_sites=self.take_scope();self.scope_captures(out.events,&[9],&mut scope_sites);for &(site,span) in &scope_sites { let name=self.text(span);if name.is_empty() {continue;}let offset_cp=self.cp(span[0]);let len_cp=self.cp(span[1])-offset_cp;
if site==9 {
...
let mut out=self.e56_c(state);
if out.ok {
let mut scope_sites=self.take_scope();self.scope_captures(out.events,&[13],&mut scope_sites);for &(site,span) in &scope_sites { let name=self.text(span);if name.is_empty() {continue;}let offset_cp=self.cp(span[0]);let len_cp=self.cp(span[1])-offset_cp;
if site==13 {
self.effect_at(ScanEffect::Declare{name,offset_cp},"lexical",offset_cp,len_cp);
}
let _=(offset_cp,len_cp); }
...
let mut out=self.e74_c(state);
if out.ok {
let mut scope_sites=self.take_scope();self.scope_captures(out.events,&[17],&mut scope_sites);for &(site,span) in &scope_sites { let name=self.text(span);if name.is_empty() {continue;}let offset_cp=self.cp(span[0]);let len_cp=self.cp(span[1])-offset_cp;
if site==17 {
self.effect_at(ScanEffect::Declare{name,offset_cp},"lexical",offset_cp,len_cp);
}
let _=(offset_cp,len_cp); }
...
let mut out=self.e92_c(state);
if out.ok {
let mut scope_sites=self.take_scope();self.scope_captures(out.events,&[21],&mut scope_sites);for &(site,span) in &scope_sites { let name=self.text(span);if name.is_empty() {continue;}let offset_cp=self.cp(span[0]);let len_cp=self.cp(span[1])-offset_cp;
if site==21 {
self.effect_at(ScanEffect::Declare{name,offset_cp},"lexical",offset_cp,len_cp);
}
let _=(offset_cp,len_cp); }
...
let mut out=self.e183_c(state);
if out.ok {
self.effect_scope_at(ScanEffect::Leave,"lexical",self.cp(out.state.consumed),0,"NumberMethodDeclaration");
let mut scope_sites=self.take_scope();self.scope_captures(out.events,&[25],&mut scope_sites);for &(site,span) in &scope_sites { let name=self.text(span);if name.is_empty() {continue;}let offset_cp=self.cp(span[0]);let len_cp=self.cp(span[1])-offset_cp;
if site==25 {
self.effect_at(ScanEffect::Declare{name,offset_cp},"lexical",offset_cp,len_cp);
}
let _=(offset_cp,len_cp); }
...
let mut out=self.e193_c(state);
if out.ok {
self.effect_scope_at(ScanEffect::Leave,"lexical",self.cp(out.state.consumed),0,"StringMethodDeclaration");
let mut scope_sites=self.take_scope();self.scope_captures(out.events,&[28],&mut scope_sites);for &(site,span) in &scope_sites { let name=self.text(span);if name.is_empty() {continue;}let offset_cp=self.cp(span[0]);let len_cp=self.cp(span[1])-offset_cp;
if site==28 {
self.effect_at(ScanEffect::Declare{name,offset_cp},"lexical",offset_cp,len_cp);
}
let _=(offset_cp,len_cp); }
...
let mut out=self.e203_c(state);
if out.ok {
self.effect_scope_at(ScanEffect::Leave,"lexical",self.cp(out.state.consumed),0,"BooleanMethodDeclaration");
let mut scope_sites=self.take_scope();self.scope_captures(out.events,&[31],&mut scope_sites);for &(site,span) in &scope_sites { let name=self.text(span);if name.is_empty() {continue;}let offset_cp=self.cp(span[0]);let len_cp=self.cp(span[1])-offset_cp;
if site==31 {
self.effect_at(ScanEffect::Declare{name,offset_cp},"lexical",offset_cp,len_cp);
}
let _=(offset_cp,len_cp); }
...
let mut out=self.e213_c(state);
if out.ok {
self.effect_scope_at(ScanEffect::Leave,"lexical",self.cp(out.state.consumed),0,"ObjectMethodDeclaration");
let mut scope_sites=self.take_scope();self.scope_captures(out.events,&[34],&mut scope_sites);for &(site,span) in &scope_sites { let name=self.text(span);if name.is_empty() {continue;}let offset_cp=self.cp(span[0]);let len_cp=self.cp(span[1])-offset_cp;
if site==34 {
self.effect_at(ScanEffect::Declare{name,offset_cp},"lexical",offset_cp,len_cp);
}
let _=(offset_cp,len_cp); }
...
let mut out=self.e229_c(state);
if out.ok {
let mut scope_sites=self.take_scope();self.scope_captures(out.events,&[39],&mut scope_sites);for &(site,span) in &scope_sites { let name=self.text(span);if name.is_empty() {continue;}let offset_cp=self.cp(span[0]);let len_cp=self.cp(span[1])-offset_cp;
if site==39 {
self.effect_at(ScanEffect::Declare{name,offset_cp},"lexical",offset_cp,len_cp);
}
let _=(offset_cp,len_cp); }
...
let mut out=self.e345_c(state);
if out.ok {
let mut scope_sites=self.take_scope();self.scope_captures(out.events,&[57],&mut scope_sites);for &(site,span) in &scope_sites { let name=self.text(span);if name.is_empty() {continue;}let offset_cp=self.cp(span[0]);let len_cp=self.cp(span[1])-offset_cp;
if site==57 {
...
let mut out=self.e1004_c(state);
if out.ok {
let end=out.state.consumed;
...
let mut out=self.e1008_c(state);
if out.ok {
let end=out.state.consumed;
...
let mut out=self.e1012_c(state);
if out.ok {
let end=out.state.consumed;
...
if out.ok {
let mut scope_sites=self.take_scope();self.scope_captures(out.events,&[238],&mut scope_sites);for &(site,span) in &scope_sites { let name=self.text(span);if name.is_empty() {continue;}let offset_cp=self.cp(span[0]);let len_cp=self.cp(span[1])-offset_cp;
if site==238 {
self.effect_at(ScanEffect::Use{name,offset_cp,len_cp},"lexical",offset_cp,len_cp);
}
let _=(offset_cp,len_cp); }
...
let mut out=self.e1022_c(state);
if out.ok {
let end=out.state.consumed;
...
let mut out=self.e1033_c(state);
if out.ok {
let end=out.state.consumed;
...
let trivia=self.trivia_0::<false>(out.state);out=self.combine(out,trivia);
let child=self.e19_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_0::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e21_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let trivia=self.trivia_0::<false>(out.state);out=self.combine(out,trivia);
let child=self.e23_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_0::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e29_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e30_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
}
let _=count;
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e32_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let mark=self.mark();
let key=Key {expression:33,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b33_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
out
...
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,118))) {self.guard_e34_c(out.state.begin())} else {self.e34_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,118))) {self.guard_e35_c(out.state.begin())} else {self.e35_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
...
let mark=self.mark();
let key=Key {expression:38,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b38_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e39_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e43_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mut out=Step::yes(state);
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
self.restore(mark);let mut child=self.e41_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e42_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e50_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e51_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e54_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let mark=self.mark();
let key=Key {expression:56,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b56_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e62_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mut out=Step::yes(state);
out=self.e58_c(out.state);
out
}
fn e58_c(&mut self,state:State)->Step {
let mut out=self.b58_c(state);
if out.ok {
} else {self.display_failures(state.consumed.max(state.matched),&["'var'", "'variable'"]);}
out
}
...
let mut out=Step::yes(state);
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
self.restore(mark);let mut child=self.e59_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e60_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e64_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e68_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e69_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let mark=self.mark();
let key=Key {expression:74,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b74_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e78_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e82_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e87_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let mark=self.mark();
let key=Key {expression:92,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b92_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e98_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e100_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e104_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e105_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e108_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e112_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let mut out=Step::yes(state);
out=self.e114_c(out.state);
out
}
fn e114_c(&mut self,state:State)->Step {
let mut out=self.b114_c(state);
if out.ok {
} else {self.display_failures(state.consumed.max(state.matched),&["'Boolean'", "'Number'", "'Object'", "'String'", "'boolean'", "'number'", "'object'", "'string'"]);}
out
}
...
let mut out=Step::yes(state);
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
self.restore(mark);let mut child=self.e115_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e116_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e124_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e125_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let mut out=Step::yes(state);
out=self.e127_c(out.state);
out
}
fn e127_c(&mut self,state:State)->Step {
let mut out=self.b127_c(state);
if out.ok {
} else {self.display_failures(state.consumed.max(state.matched),&["'Float'", "'Number'", "'float'", "'number'"]);}
out
}
...
let mut out=Step::yes(state);
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
self.restore(mark);let mut child=self.e128_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e129_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e133_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e134_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let mut out=Step::yes(state);
out=self.e136_c(out.state);
out
}
fn e136_c(&mut self,state:State)->Step {
let mut out=self.b136_c(state);
if out.ok {
} else {self.display_failures(state.consumed.max(state.matched),&["'String'", "'string'"]);}
out
}
...
let mut out=Step::yes(state);
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
self.restore(mark);let mut child=self.e137_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e138_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e140_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e141_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let mut out=Step::yes(state);
out=self.e143_c(out.state);
out
}
fn e143_c(&mut self,state:State)->Step {
let mut out=self.b143_c(state);
if out.ok {
} else {self.display_failures(state.consumed.max(state.matched),&["'Boolean'", "'boolean'"]);}
out
}
...
let mut out=Step::yes(state);
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
self.restore(mark);let mut child=self.e144_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e145_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e147_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e148_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let mut out=Step::yes(state);
out=self.e150_c(out.state);
out
}
fn e150_c(&mut self,state:State)->Step {
let mut out=self.b150_c(state);
if out.ok {
} else {self.display_failures(state.consumed.max(state.matched),&["'Object'", "'object'"]);}
out
}
...
let mut out=Step::yes(state);
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
self.restore(mark);let mut child=self.e151_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e152_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e154_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e155_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:157,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b157_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
out
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e159_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e163_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e166_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let mark=self.mark();
let key=Key {expression:168,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b168_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e170_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e171_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
}
let _=count;
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e173_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let mark=self.mark();
let key=Key {expression:174,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b174_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e176_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mut out=Step::yes(state);
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
self.restore(mark);let mut child=self.e179_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e180_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
...
let mark=self.mark();
let key=Key {expression:183,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b183_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e184_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e185_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e190_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e188_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let mark=self.mark();
let key=Key {expression:193,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b193_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e195_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e200_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e198_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let mark=self.mark();
let key=Key {expression:203,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b203_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e205_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e210_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e208_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let mark=self.mark();
let key=Key {expression:213,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b213_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e215_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e220_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e218_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let mark=self.mark();
let key=Key {expression:223,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b223_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e225_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
fn e224_c(&mut self,state:State)->Step {
let mut out=self.b224_c(state);
if out.ok {
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e226_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
}
let _=count;
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e228_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let mark=self.mark();
let key=Key {expression:229,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b229_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
out
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e231_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e233_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e235_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let mut out=Step::yes(state);
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
self.restore(mark);let mut child=self.e237_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e238_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e240_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let mark=self.mark();
let key=Key {expression:245,state,matched_mode:false,version:0};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b245_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
out
...
let ct=self.input.cp_at(self.skip_0::<false>(out.state.begin()).position::<false>()).map(|(c,_)|c);
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(c0.is_some_and(|c| matches!(c as u32,102|110))) {self.guard_e246_c(out.state.begin())} else {self.e246_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,115))) {self.guard_e247_c(out.state.begin())} else {self.e247_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,98))) {self.guard_e248_c(out.state.begin())} else {self.e248_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,111))) {self.guard_e249_c(out.state.begin())} else {self.e249_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
let mark=self.mark();
let key=Key {expression:250,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b250_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e251_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e252_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e257_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e258_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e268_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e253_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e255_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e256_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e259_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let mut out=Step::yes(state);
out=self.e261_c(out.state);
out
}
fn e261_c(&mut self,state:State)->Step {
let mut out=self.b261_c(state);
if out.ok {
} else {self.display_failures(state.consumed.max(state.matched),&["'#'"]);}
out
}
...
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
let ct=self.input.cp_at(self.skip_0::<false>(out.state.begin()).position::<false>()).map(|(c,_)|c);
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,65..=90|95|97..=122))) {self.guard_e262_c(out.state.begin())} else {self.e262_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e263_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e264_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e269_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let mark=self.mark();
let key=Key {expression:271,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b271_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e273_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e286_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e293_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e294_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mut out=Step::yes(state);
out=self.e274_c(out.state);
out
}
fn e274_c(&mut self,state:State)->Step {
let mut out=self.b274_c(state);
if out.ok {
} else {self.display_failures(state.consumed.max(state.matched),&["':'"]);}
out
}
...
let mut out=Step::yes(state);
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
self.restore(mark);let mut child=self.e275_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e284_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e276_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e281_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e282_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e277_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e279_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e280_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e283_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let mut out=Step::yes(state);
out=self.e287_c(out.state);
out
}
fn e287_c(&mut self,state:State)->Step {
let mut out=self.b287_c(state);
if out.ok {
} else {self.display_failures(state.consumed.max(state.matched),&["'#'"]);}
out
}
...
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
let ct=self.input.cp_at(self.skip_0::<false>(out.state.begin()).position::<false>()).map(|(c,_)|c);
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,65..=90|95|97..=122))) {self.guard_e288_c(out.state.begin())} else {self.e288_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e289_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e290_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e295_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let mark=self.mark();
let key=Key {expression:297,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b297_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e299_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e304_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e305_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e315_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e300_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e302_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e303_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e306_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let mut out=Step::yes(state);
out=self.e308_c(out.state);
out
}
fn e308_c(&mut self,state:State)->Step {
let mut out=self.b308_c(state);
if out.ok {
} else {self.display_failures(state.consumed.max(state.matched),&["'#'"]);}
out
}
...
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
let ct=self.input.cp_at(self.skip_0::<false>(out.state.begin()).position::<false>()).map(|(c,_)|c);
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,65..=90|95|97..=122))) {self.guard_e309_c(out.state.begin())} else {self.e309_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e310_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e311_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e316_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let mark=self.mark();
let key=Key {expression:318,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b318_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e320_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e325_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e326_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e336_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e321_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e323_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e324_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e327_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let mut out=Step::yes(state);
out=self.e329_c(out.state);
out
}
fn e329_c(&mut self,state:State)->Step {
let mut out=self.b329_c(state);
if out.ok {
} else {self.display_failures(state.consumed.max(state.matched),&["'#'"]);}
out
}
...
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
let ct=self.input.cp_at(self.skip_0::<false>(out.state.begin()).position::<false>()).map(|(c,_)|c);
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,65..=90|95|97..=122))) {self.guard_e330_c(out.state.begin())} else {self.e330_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e331_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e332_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e337_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
let ct=self.input.cp_at(self.skip_0::<false>(out.state.begin()).position::<false>()).map(|(c,_)|c);
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,99))) {self.guard_e340_c(out.state.begin())} else {self.e340_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e344_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e341_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e342_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e343_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let mark=self.mark();
let key=Key {expression:345,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b345_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e346_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e347_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e350_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let mark=self.mark();
let key=Key {expression:352,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b352_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e354_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:358,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b358_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
out
...
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
let ct=self.input.cp_at(self.skip_0::<false>(out.state.begin()).position::<false>()).map(|(c,_)|c);
self.restore(mark);let mut child=self.e359_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,36|40|43|45..=46|48..=57|97|99|101..=102|105|108..=109|112|114..=116))) {self.guard_e360_c(out.state.begin())} else {self.e360_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e361_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e362_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
let mark=self.mark();
let key=Key {expression:363,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b363_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e364_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e365_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e366_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
}
let _=count;
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e368_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let mark=self.mark();
let key=Key {expression:369,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b369_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e371_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e372_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
}
let _=count;
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e374_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let mark=self.mark();
let key=Key {expression:375,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b375_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e377_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e378_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
}
let _=count;
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e380_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
0=>Some(self.e382_c(out.state.begin())),
1=>Some(self.e383_c(out.state.begin())),
_=>None,};
...
self.restore(mark);let mut child=self.e382_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e383_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
Some('/')=>1usize,
_=>usize::MAX,};let candidate=match predicted {
0=>Some(self.e385_c(out.state.begin())),
1=>Some(self.e386_c(out.state.begin())),
_=>None,};
if !DIAG && self.options.predict && predicted==usize::MAX {self.restore(mark);out.ok=false;out.diag=diagnostic;break 'choice out;}
if let Some(mut child)=candidate {if child.ok {
if predicted>0 {let d=self.diag_fail(out.state.begin().position::<false>(),"*");diagnostic=self.diag_join(diagnostic,d);}
...
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e386_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
let mark=self.mark();
let key=Key {expression:387,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b387_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
out
...
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
let ct=self.input.cp_at(self.skip_0::<false>(out.state.begin()).position::<false>()).map(|(c,_)|c);
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,115))) {self.guard_e388_c(out.state.begin())} else {self.e388_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,99))) {self.guard_e389_c(out.state.begin())} else {self.e389_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,116))) {self.guard_e390_c(out.state.begin())} else {self.e390_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,115))) {self.guard_e391_c(out.state.begin())} else {self.e391_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,109))) {self.guard_e392_c(out.state.begin())} else {self.e392_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,109))) {self.guard_e393_c(out.state.begin())} else {self.e393_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,114))) {self.guard_e394_c(out.state.begin())} else {self.e394_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,97))) {self.guard_e395_c(out.state.begin())} else {self.e395_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
...
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,99))) {self.guard_e397_c(out.state.begin())} else {self.e397_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,102))) {self.guard_e398_c(out.state.begin())} else {self.e398_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,112))) {self.guard_e399_c(out.state.begin())} else {self.e399_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,108))) {self.guard_e400_c(out.state.begin())} else {self.e400_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,101))) {self.guard_e401_c(out.state.begin())} else {self.e401_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
let mark=self.mark();
let key=Key {expression:402,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b402_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e403_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e404_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:407,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b407_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e409_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:412,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b412_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e414_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:417,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b417_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e419_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:422,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b422_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e424_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e427_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
}
let _=count;
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e429_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let mark=self.mark();
let key=Key {expression:431,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b431_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e433_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e436_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
}
let _=count;
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e438_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e442_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:444,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b444_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e446_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:449,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b449_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e451_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:454,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b454_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e456_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:459,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b459_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e461_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:464,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b464_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e466_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:471,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b471_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e473_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:476,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b476_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e478_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:481,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b481_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e483_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:488,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b488_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
out
...
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
let ct=self.input.cp_at(self.skip_0::<false>(out.state.begin()).position::<false>()).map(|(c,_)|c);
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,40))) {self.guard_e489_c(out.state.begin())} else {self.e489_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,109))) {self.guard_e490_c(out.state.begin())} else {self.e490_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,105))) {self.guard_e491_c(out.state.begin())} else {self.e491_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,97|99|101..=102|108..=109|112|114..=116))) {self.guard_e492_c(out.state.begin())} else {self.e492_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,116))) {self.guard_e493_c(out.state.begin())} else {self.e493_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,36))) {self.guard_e494_c(out.state.begin())} else {self.e494_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,108))) {self.guard_e495_c(out.state.begin())} else {self.e495_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,108))) {self.guard_e496_c(out.state.begin())} else {self.e496_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,101))) {self.guard_e497_c(out.state.begin())} else {self.e497_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e498_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,36))) {self.guard_e499_c(out.state.begin())} else {self.e499_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,99|105))) {self.guard_e500_c(out.state.begin())} else {self.e500_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,40))) {self.guard_e501_c(out.state.begin())} else {self.e501_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e502_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e503_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:505,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b505_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e507_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:510,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b510_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e512_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:515,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b515_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e517_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:520,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b520_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e522_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:525,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b525_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e527_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:530,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b530_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e532_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:535,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b535_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e537_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:540,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b540_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e542_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:545,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b545_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e547_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:550,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b550_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e552_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e557_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
}
let _=count;
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e559_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let mark=self.mark();
let key=Key {expression:561,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b561_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e563_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e568_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
}
let _=count;
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e570_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let mark=self.mark();
let key=Key {expression:572,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b572_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e574_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e579_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
}
let _=count;
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e581_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let mark=self.mark();
let key=Key {expression:583,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b583_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e585_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e589_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
}
let _=count;
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e591_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let mark=self.mark();
let key=Key {expression:593,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b593_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e595_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e599_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
}
let _=count;
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e601_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let mark=self.mark();
let key=Key {expression:603,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b603_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e605_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e609_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
}
let _=count;
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e611_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let mark=self.mark();
let key=Key {expression:613,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b613_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e615_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e619_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
}
let _=count;
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e621_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let mark=self.mark();
let key=Key {expression:623,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b623_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
out
...
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
let ct=self.input.cp_at(self.skip_0::<false>(out.state.begin()).position::<false>()).map(|(c,_)|c);
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,116))) {self.guard_e624_c(out.state.begin())} else {self.e624_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,116))) {self.guard_e625_c(out.state.begin())} else {self.e625_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
...
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,36))) {self.guard_e627_c(out.state.begin())} else {self.e627_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
let mark=self.mark();
let key=Key {expression:628,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b628_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e629_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e630_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:633,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b633_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e635_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:640,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b640_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e642_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mut out=Step::yes(state);
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
self.restore(mark);let mut child=self.e652_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e653_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
...
let mark=self.mark();
let key=Key {expression:659,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b659_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
out
...
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
let ct=self.input.cp_at(self.skip_0::<false>(out.state.begin()).position::<false>()).map(|(c,_)|c);
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,101))) {self.guard_e660_c(out.state.begin())} else {self.e660_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,40))) {self.guard_e661_c(out.state.begin())} else {self.e661_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,40))) {self.guard_e665_c(out.state.begin())} else {self.e665_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,116))) {self.guard_e666_c(out.state.begin())} else {self.e666_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,116))) {self.guard_e667_c(out.state.begin())} else {self.e667_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
...
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,36))) {self.guard_e669_c(out.state.begin())} else {self.e669_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,36))) {self.guard_e670_c(out.state.begin())} else {self.e670_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
...
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e672_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
...
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,99|105))) {self.guard_e674_c(out.state.begin())} else {self.e674_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e662_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e663_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:675,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b675_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e676_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let p0=cur.state;let child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,34|36|39..=40|99|101|105|116))) {self.guard_e683_c(cur.state)} else {self.e683_c(cur.state)};let ev0=child.events;let _=(p0,ev0);cur=self.combine(cur,child);
if !cur.ok {
...
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
let p4=cur.state;
let child=self.e684_c(cur.state);let ev3=child.events;let _=(p4,ev3);cur=self.combine(cur,child);
if !cur.ok {
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
...
let p8=cur.state;
let child=self.e685_c(cur.state);let ev7=child.events;let _=(p8,ev7);cur=self.combine(cur,child);
if !cur.ok {
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
...
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
let p12=cur.state;
let child=self.e686_c(cur.state);let ev11=child.events;let _=(p12,ev11);cur=self.combine(cur,child);
if !cur.ok {
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
...
let p16=cur.state;
let child=self.e687_c(cur.state);let ev15=child.events;let _=(p16,ev15);cur=self.combine(cur,child);
if !cur.ok {
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
...
'f19: {
let child=self.e688_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f19;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
let child=self.e689_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f19;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
...
'f20: {
let child=self.e697_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f20;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
cur.state=st.commit(cur.state);
cur.events=self.event(Event::Values{span:[entry.consumed,cur.state.consumed],child:cur.events,wrap:false});
self.retag(ev0,Some(133),None);
self.retag(ev3,None,Some(693));
self.retag(ev7,Some(134),None);
self.retag(ev11,None,Some(695));
self.retag(ev15,Some(135),None);
diagnostic=self.diag_join(diagnostic,cur.diag);break 'factored cur;
}
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
diagnostic=self.diag_join(diagnostic,cur.diag);
...
'f21: {
let child=self.e703_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f21;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
let child=self.e704_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f21;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
let child=self.e705_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f21;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
cur.state=st.commit(cur.state);
cur.events=self.event(Event::Values{span:[entry.consumed,cur.state.consumed],child:cur.events,wrap:false});
self.retag(ev0,Some(136),None);
self.retag(ev3,None,Some(700));
self.retag(ev7,Some(137),None);
self.retag(ev11,None,Some(702));
diagnostic=self.diag_join(diagnostic,cur.diag);break 'factored cur;
}
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
diagnostic=self.diag_join(diagnostic,cur.diag);
self.restore(m13);cur=b14;
'f22: {
let child=self.e711_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f22;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
cur.state=st.commit(cur.state);
cur.events=self.event(Event::Values{span:[entry.consumed,cur.state.consumed],child:cur.events,wrap:false});
self.retag(ev0,Some(139),None);
self.retag(ev3,None,Some(708));
self.retag(ev7,Some(140),None);
self.retag(ev11,None,Some(710));
diagnostic=self.diag_join(diagnostic,cur.diag);break 'factored cur;
}
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
diagnostic=self.diag_join(diagnostic,cur.diag);
...
let p24=cur.state;
let child=self.e715_c(cur.state);let ev23=child.events;let _=(p24,ev23);cur=self.combine(cur,child);
if !cur.ok {
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
...
let p28=cur.state;
let child=self.e716_c(cur.state);let ev27=child.events;let _=(p28,ev27);cur=self.combine(cur,child);
if !cur.ok {
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
...
'f31: {
let child=self.e717_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f31;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
let child=self.e718_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f31;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
...
'f32: {
let child=self.e725_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f32;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
cur.state=st.commit(cur.state);
cur.events=self.event(Event::Values{span:[entry.consumed,cur.state.consumed],child:cur.events,wrap:false});
self.retag(ev0,Some(144),None);
self.retag(ev3,None,Some(722));
self.retag(ev23,None,Some(723));
self.retag(ev27,Some(145),None);
diagnostic=self.diag_join(diagnostic,cur.diag);break 'factored cur;
}
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
diagnostic=self.diag_join(diagnostic,cur.diag);
...
'f33: {
let child=self.e730_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f33;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
let child=self.e731_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f33;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
...
self.retag(ev3,None,Some(728));
self.retag(ev23,None,Some(729));
diagnostic=self.diag_join(diagnostic,cur.diag);break 'factored cur;
}
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
diagnostic=self.diag_join(diagnostic,cur.diag);
self.restore(m25);cur=b26;
'f34: {
let child=self.e737_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f34;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
cur.state=st.commit(cur.state);
...
self.retag(ev3,None,Some(735));
self.retag(ev23,None,Some(736));
diagnostic=self.diag_join(diagnostic,cur.diag);break 'factored cur;
}
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
diagnostic=self.diag_join(diagnostic,cur.diag);
...
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,34|36|39..=40|99|101|105|116))) {self.guard_e682_c(out.state.begin())} else {self.e682_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,34|36|39..=40|99|101|105|116))) {self.guard_e691_c(out.state.begin())} else {self.e691_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,34|36|39..=40|99|101|105|116))) {self.guard_e698_c(out.state.begin())} else {self.e698_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,34|36|39..=40|99|101|105|116))) {self.guard_e706_c(out.state.begin())} else {self.e706_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,34|36|39..=40|99|101|105|116))) {self.guard_e712_c(out.state.begin())} else {self.e712_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,34|36|39..=40|99|101|105|116))) {self.guard_e720_c(out.state.begin())} else {self.e720_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,34|36|39..=40|99|101|105|116))) {self.guard_e726_c(out.state.begin())} else {self.e726_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,34|36|39..=40|99|101|105|116))) {self.guard_e733_c(out.state.begin())} else {self.e733_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e683_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e684_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e693_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e700_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e708_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e714_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e722_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e728_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e735_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:738,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b738_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
out
...
cur.state=st.begin();
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
let p0=cur.state;let child=self.e740_c(cur.state);let ev0=child.events;let _=(p0,ev0);cur=self.combine(cur,child);
if !cur.ok {
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
self.display_failures(p0.consumed.max(p0.matched),&["__CaptureSite"]);
diagnostic=self.diag_join(diagnostic,cur.diag);
...
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
let p4=cur.state;
let child=self.e741_c(cur.state);let ev3=child.events;let _=(p4,ev3);cur=self.combine(cur,child);
if !cur.ok {
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
self.display_failures(p4.consumed.max(p4.matched),&["'['"]);
diagnostic=self.diag_join(diagnostic,cur.diag);
...
let m5=self.mark();let b6=cur;
let p8=cur.state;
let child=self.e742_c(cur.state);let ev7=child.events;let _=(p8,ev7);cur=self.combine(cur,child);
if !cur.ok {
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
self.display_failures(p8.consumed.max(p8.matched),&["__CaptureSite"]);
diagnostic=self.diag_join(diagnostic,cur.diag);
...
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
let p12=cur.state;
let child=self.e743_c(cur.state);let ev11=child.events;let _=(p12,ev11);cur=self.combine(cur,child);
if !cur.ok {
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
self.display_failures(p12.consumed.max(p12.matched),&["':'"]);
diagnostic=self.diag_join(diagnostic,cur.diag);
...
let m13=self.mark();let b14=cur;
let p16=cur.state;
let child=self.e744_c(cur.state);let ev15=child.events;let _=(p16,ev15);cur=self.combine(cur,child);
if !cur.ok {
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
self.display_failures(p16.consumed.max(p16.matched),&["__CaptureSite"]);
diagnostic=self.diag_join(diagnostic,cur.diag);
...
let m17=self.mark();let b18=cur;
'f19: {
let child=self.e745_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f19;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
let child=self.e746_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f19;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
...
self.restore(m17);cur=b18;
'f20: {
let child=self.e754_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f20;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
cur.state=st.commit(cur.state);
cur.events=self.event(Event::Values{span:[entry.consumed,cur.state.consumed],child:cur.events,wrap:false});
self.retag(ev0,Some(153),None);
self.retag(ev3,None,Some(750));
self.retag(ev7,Some(154),None);
self.retag(ev11,None,Some(752));
self.retag(ev15,Some(155),None);
diagnostic=self.diag_join(diagnostic,cur.diag);break 'factored cur;
}
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
diagnostic=self.diag_join(diagnostic,cur.diag);
...
self.restore(m13);cur=b14;
'f21: {
let child=self.e760_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f21;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
let child=self.e761_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f21;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
let child=self.e762_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f21;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
cur.state=st.commit(cur.state);
cur.events=self.event(Event::Values{span:[entry.consumed,cur.state.consumed],child:cur.events,wrap:false});
self.retag(ev0,Some(156),None);
self.retag(ev3,None,Some(757));
self.retag(ev7,Some(157),None);
self.retag(ev11,None,Some(759));
diagnostic=self.diag_join(diagnostic,cur.diag);break 'factored cur;
}
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
diagnostic=self.diag_join(diagnostic,cur.diag);
self.restore(m13);cur=b14;
'f22: {
let child=self.e768_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f22;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
cur.state=st.commit(cur.state);
cur.events=self.event(Event::Values{span:[entry.consumed,cur.state.consumed],child:cur.events,wrap:false});
self.retag(ev0,Some(159),None);
self.retag(ev3,None,Some(765));
self.retag(ev7,Some(160),None);
self.retag(ev11,None,Some(767));
diagnostic=self.diag_join(diagnostic,cur.diag);break 'factored cur;
}
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
diagnostic=self.diag_join(diagnostic,cur.diag);
...
self.restore(m5);cur=b6;
let p24=cur.state;
let child=self.e772_c(cur.state);let ev23=child.events;let _=(p24,ev23);cur=self.combine(cur,child);
if !cur.ok {
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
self.display_failures(p24.consumed.max(p24.matched),&["':'"]);
diagnostic=self.diag_join(diagnostic,cur.diag);
...
let m25=self.mark();let b26=cur;
let p28=cur.state;
let child=self.e773_c(cur.state);let ev27=child.events;let _=(p28,ev27);cur=self.combine(cur,child);
if !cur.ok {
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
self.display_failures(p28.consumed.max(p28.matched),&["__CaptureSite"]);
diagnostic=self.diag_join(diagnostic,cur.diag);
...
let m29=self.mark();let b30=cur;
'f31: {
let child=self.e774_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f31;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
let child=self.e775_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f31;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
...
self.restore(m29);cur=b30;
'f32: {
let child=self.e782_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f32;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
cur.state=st.commit(cur.state);
cur.events=self.event(Event::Values{span:[entry.consumed,cur.state.consumed],child:cur.events,wrap:false});
self.retag(ev0,Some(164),None);
self.retag(ev3,None,Some(779));
self.retag(ev23,None,Some(780));
self.retag(ev27,Some(165),None);
diagnostic=self.diag_join(diagnostic,cur.diag);break 'factored cur;
}
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
diagnostic=self.diag_join(diagnostic,cur.diag);
...
self.restore(m25);cur=b26;
'f33: {
let child=self.e787_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f33;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
let child=self.e788_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f33;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
...
self.retag(ev3,None,Some(785));
self.retag(ev23,None,Some(786));
diagnostic=self.diag_join(diagnostic,cur.diag);break 'factored cur;
}
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
diagnostic=self.diag_join(diagnostic,cur.diag);
self.restore(m25);cur=b26;
'f34: {
let child=self.e794_c(cur.state);cur=self.combine(cur,child);if !cur.ok {break 'f34;}
let trivia=self.trivia_1::<false>(cur.state);cur=self.combine(cur,trivia);
cur.state=st.commit(cur.state);
cur.events=self.event(Event::Values{span:[entry.consumed,cur.state.consumed],child:cur.events,wrap:true});
self.retag(ev0,Some(168),None);
self.retag(ev3,None,Some(792));
self.retag(ev23,None,Some(793));
diagnostic=self.diag_join(diagnostic,cur.diag);break 'factored cur;
}
self.display_failures(entry.consumed.max(entry.matched),&["':'", "'['", "']'"]);
diagnostic=self.diag_join(diagnostic,cur.diag);
...
self.restore(mark);let mut child=self.e739_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e748_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e755_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e763_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e769_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e777_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e783_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e790_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e740_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e741_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e750_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e757_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e765_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e771_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e779_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e785_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e792_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:795,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b795_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
out
...
let mut out=Step::yes(state);
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
self.restore(mark);let mut child=self.e796_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e797_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
let mark=self.mark();
let key=Key {expression:798,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b798_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e799_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e800_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e801_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
}
let _=count;
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e803_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let mark=self.mark();
let key=Key {expression:804,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b804_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e806_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
let ct=self.input.cp_at(self.skip_0::<false>(out.state.begin()).position::<false>()).map(|(c,_)|c);
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,109))) {self.guard_e809_c(out.state.begin())} else {self.e809_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,105))) {self.guard_e810_c(out.state.begin())} else {self.e810_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,40))) {self.guard_e811_c(out.state.begin())} else {self.e811_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,36))) {self.guard_e812_c(out.state.begin())} else {self.e812_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e813_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,40))) {self.guard_e814_c(out.state.begin())} else {self.e814_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,101))) {self.guard_e815_c(out.state.begin())} else {self.e815_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,116))) {self.guard_e816_c(out.state.begin())} else {self.e816_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,116))) {self.guard_e817_c(out.state.begin())} else {self.e817_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
...
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,36))) {self.guard_e819_c(out.state.begin())} else {self.e819_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,36))) {self.guard_e820_c(out.state.begin())} else {self.e820_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
...
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e822_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
...
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,99|105))) {self.guard_e824_c(out.state.begin())} else {self.e824_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e826_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e827_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e831_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e832_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mut out=Step::yes(state);
out=self.e828_c(out.state);
out
}
fn e828_c(&mut self,state:State)->Step {
let mut out=self.b828_c(state);
if out.ok {
} else {self.display_failures(state.consumed.max(state.matched),&["'String'", "'string'"]);}
out
}
...
let mut out=Step::yes(state);
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
self.restore(mark);let mut child=self.e829_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e830_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e836_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mut out=Step::yes(state);
out=self.e839_c(out.state);
out
}
fn e839_c(&mut self,state:State)->Step {
let mut out=self.b839_c(state);
if out.ok {
} else {self.display_failures(state.consumed.max(state.matched),&["'String'", "'string'"]);}
out
}
...
let mut out=Step::yes(state);
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
self.restore(mark);let mut child=self.e840_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e841_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
let mark=self.mark();
let key=Key {expression:842,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b842_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e843_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e844_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
fn e843_c(&mut self,state:State)->Step {
let mut out=self.b843_c(state);
if out.ok {
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e845_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
}
let _=count;
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e847_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let mark=self.mark();
let key=Key {expression:848,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b848_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e850_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e851_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
}
let _=count;
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e853_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let mark=self.mark();
let key=Key {expression:854,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b854_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e856_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e857_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
}
let _=count;
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e859_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let mark=self.mark();
let key=Key {expression:860,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b860_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:true});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e862_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:865,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b865_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
out
...
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
let ct=self.input.cp_at(self.skip_0::<false>(out.state.begin()).position::<false>()).map(|(c,_)|c);
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,110))) {self.guard_e866_c(out.state.begin())} else {self.e866_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,105))) {self.guard_e867_c(out.state.begin())} else {self.e867_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,109))) {self.guard_e868_c(out.state.begin())} else {self.e868_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,101))) {self.guard_e869_c(out.state.begin())} else {self.e869_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e870_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,36|116))) {self.guard_e871_c(out.state.begin())} else {self.e871_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,36|116))) {self.guard_e872_c(out.state.begin())} else {self.e872_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
...
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,115))) {self.guard_e874_c(out.state.begin())} else {self.e874_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,101))) {self.guard_e875_c(out.state.begin())} else {self.e875_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,99))) {self.guard_e876_c(out.state.begin())} else {self.e876_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,105))) {self.guard_e877_c(out.state.begin())} else {self.e877_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,105))) {self.guard_e878_c(out.state.begin())} else {self.e878_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
...
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e880_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e881_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,36))) {self.guard_e882_c(out.state.begin())} else {self.e882_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,99|105))) {self.guard_e883_c(out.state.begin())} else {self.e883_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,40))) {self.guard_e884_c(out.state.begin())} else {self.e884_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e885_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e886_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:888,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b888_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e890_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
let ct=self.input.cp_at(self.skip_0::<false>(out.state.begin()).position::<false>()).map(|(c,_)|c);
self.restore(mark);let mut child=self.e893_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,36|40|43|45..=46|48..=57|97|99|101..=102|105|108..=109|112|114..=116))) {self.guard_e894_c(out.state.begin())} else {self.e894_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e895_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e896_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
let mark=self.mark();
let key=Key {expression:897,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b897_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e898_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e899_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mut out=Step::yes(state);
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
self.restore(mark);let mut child=self.e902_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e903_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
let mark=self.mark();
let key=Key {expression:904,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b904_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e905_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e906_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mut out=Step::yes(state);
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
self.restore(mark);let mut child=self.e909_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e910_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
...
let mark=self.mark();
let key=Key {expression:915,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b915_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
out
...
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
let ct=self.input.cp_at(self.skip_0::<false>(out.state.begin()).position::<false>()).map(|(c,_)|c);
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,36|40|43|45..=46|48..=57|97|99|101..=102|105|108..=109|112|114..=116))) {self.guard_e916_c(out.state.begin())} else {self.e916_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e917_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e918_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,101))) {self.guard_e919_c(out.state.begin())} else {self.e919_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,36))) {self.guard_e920_c(out.state.begin())} else {self.e920_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,99|105))) {self.guard_e921_c(out.state.begin())} else {self.e921_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
let mark=self.mark();
let key=Key {expression:922,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b922_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e923_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e924_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:934,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b934_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
out
...
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
let ct=self.input.cp_at(self.skip_0::<false>(out.state.begin()).position::<false>()).map(|(c,_)|c);
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,36|40|43|45..=46|48..=57|97|99|101..=102|105|108..=109|112|114..=116))) {self.guard_e935_c(out.state.begin())} else {self.e935_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e936_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e937_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,36|40|43|45..=46|48..=57|97|99|101..=102|105|108..=109|112|114..=116))) {self.guard_e938_c(out.state.begin())} else {self.e938_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
...
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,99|105))) {self.guard_e942_c(out.state.begin())} else {self.e942_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
let mark=self.mark();
let key=Key {expression:943,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b943_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e944_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e945_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mark=self.mark();
let key=Key {expression:951,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b951_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e953_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e956_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
}
let _=count;
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e958_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let mark=self.mark();
let key=Key {expression:962,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b962_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e964_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e968_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e974_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e977_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
}
let _=count;
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e979_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let mark=self.mark();
let key=Key {expression:983,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b983_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e985_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e989_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e995_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e998_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
}
let _=count;
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e1000_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
out.state=if out.ok {state.commit(out.state)} else {state};
out
}
...
let mark=self.mark();
let key=Key {expression:1004,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b1004_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
out.events=self.event(Event::Values{span:[state.consumed,out.state.consumed],child:out.events,wrap:false});
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e1006_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mut out=self.b1005_c(state);
if out.ok {
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e1010_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mut out=self.b1011_c(state);
if out.ok {
...
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e1016_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
let mut out=self.b1017_c(state);
if out.ok {
} else {self.display_failures(state.consumed.max(state.matched),&["Optional"]);}
out
}
...
out.state=state.begin();
let mut count=0usize;loop {let mark=self.mark();let before=out.state;
let mut child=self.e1018_c(out.state);out.diag=self.diag_join(out.diag,child.diag);out.state=child.state;if !child.ok {self.restore(mark);break;}
out.events=self.join(out.events,child.events);count+=1;
if before.position::<false>()==out.state.position::<false>() {break;}
if count>=1 {break;}
}
...
let mut out=self.b1018_c(state);
if out.ok {
} else {self.display_failures(state.consumed.max(state.matched),&["'as'"]);}
out
}
...
let mut out=self.b1022_c(state);
if out.ok {
} else {self.display_failures(state.consumed.max(state.matched),&[]);}
out
}
...
let mut out=Step::yes(state);
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
self.restore(mark);let mut child=self.e1023_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e1024_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
...
let mark=self.mark();
let key=Key {expression:1033,state,matched_mode:false,version:self.scope.state_version()};if let Some(hit)=self.lookup(key) {return hit;}
self.display_enter(state.consumed.max(state.matched));let mut out=self.b1033_c(state);self.display_reach(out.state.consumed.max(out.state.matched));
if out.ok {
} else {self.restore(mark);self.display_failures(state.consumed.max(state.matched),&[]);}
if (out.ok && true) || (!out.ok && true) {self.store(key,out,mark);}self.display_leave();
out
...
out='choice: {let mark=self.mark();let mut diagnostic=Diag::NONE;let mut best:Option<(Step,[usize;2])>=None;
let ct=self.input.cp_at(self.skip_0::<false>(out.state.begin()).position::<false>()).map(|(c,_)|c);
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,36|40|43|45..=46|48..=57|97|99|101..=102|105|108..=109|112|114..=116))) {self.guard_e1034_c(out.state.begin())} else {self.e1034_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e1035_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e1036_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=self.e1037_c(out.state.begin());diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,99|105))) {self.guard_e1038_c(out.state.begin())} else {self.e1038_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);let mut child=if self.options.predict && !out.state.invert && !(ct.is_some_and(|c| matches!(c as u32,40))) {self.guard_e1039_c(out.state.begin())} else {self.e1039_c(out.state.begin())};diagnostic=self.diag_join(diagnostic,child.diag);
if child.ok {child.state=out.state.commit(child.state);child.diag=diagnostic;break 'choice child;}
self.restore(mark);break 'choice if let Some((mut child,effects))=best {self.replay_effects(effects);child.diag=diagnostic;child} else {out.ok=false;out.diag=diagnostic;out};
};
out
...
let mut out=self.b1034_c(state);
if out.ok {
...
let mut out=self.b1035_c(state);
if out.ok {
...
out.state=state.begin();
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e1040_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
let child=self.e1041_c(out.state);out=self.combine(out,child);if !out.ok {out.state=state;return out;}
let trivia=self.trivia_1::<false>(out.state);out=self.combine(out,trivia);
...
fn guard_e34_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e34_c(state);}
...
fn guard_e35_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e35_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
fn guard_e246_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e246_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
...
fn guard_e247_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e247_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
...
fn guard_e262_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e262_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.e28_c(t1).diag;
...
fn guard_e288_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e288_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.e28_c(t1).diag;
...
fn guard_e309_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e309_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.e28_c(t1).diag;
...
fn guard_e330_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e330_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.e28_c(t1).diag;
...
fn guard_e340_c(&mut self,state:State)->Step {
if !self.can_replay(0) {return self.e340_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
...
fn guard_e360_c(&mut self,state:State)->Step {
if !self.can_replay(6) {return self.e360_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
let d50=self.e498_c(t1.begin()).diag;
...
fn guard_e388_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e388_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
...
fn guard_e389_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e389_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
...
fn guard_e489_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e489_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
...
fn guard_e490_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e490_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
...
if !self.can_replay(2) {return self.e492_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let mut d1=Diag::NONE;
...
if !self.can_replay(1) {return self.e496_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
...
fn guard_e500_c(&mut self,state:State)->Step {
if !self.can_replay(2) {return self.e500_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
if !self.can_replay(0) {return self.e501_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.diag_fail(t1.position::<false>(),"(");
...
fn guard_e624_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e624_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
...
fn guard_e625_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e625_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
...
fn guard_e660_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e660_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.diag_fail(t1.position::<false>(),"external");
...
fn guard_e661_c(&mut self,state:State)->Step {
if !self.can_replay(0) {return self.e661_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.diag_fail(t1.position::<false>(),"(");
...
fn guard_e665_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e665_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.diag_fail(t1.position::<false>(),"(");
...
fn guard_e666_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e666_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.diag_fail(t1.position::<false>(),"toUpperCase");
...
if !self.can_replay(2) {return self.e669_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.diag_fail(t1.position::<false>(),"$");
...
if !self.can_replay(2) {return self.e670_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.diag_fail(t1.position::<false>(),"$");
...
if !self.can_replay(1) {return self.e673_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.diag_fail(t1.position::<false>(),"$");
...
fn guard_e674_c(&mut self,state:State)->Step {
if !self.can_replay(2) {return self.e674_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
fn guard_e682_c(&mut self,state:State)->Step {
if !self.can_replay(3) {return self.e682_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
let d23=self.e672_c(t1.begin()).diag;
...
fn guard_e683_c(&mut self,state:State)->Step {
if !self.can_replay(3) {return self.e683_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let mut d1=Diag::NONE;
let t2=self.trivia_1::<false>(state.begin().entered(true,true)).state;
...
let d22=self.diag_rule(68,d21);
d1=self.diag_join(d1,d22);
let d23=self.e672_c(state.begin()).diag;
...
fn guard_e691_c(&mut self,state:State)->Step {
if !self.can_replay(3) {return self.e691_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
fn guard_e698_c(&mut self,state:State)->Step {
if !self.can_replay(3) {return self.e698_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
fn guard_e706_c(&mut self,state:State)->Step {
if !self.can_replay(3) {return self.e706_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
fn guard_e712_c(&mut self,state:State)->Step {
if !self.can_replay(3) {return self.e712_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
fn guard_e720_c(&mut self,state:State)->Step {
if !self.can_replay(3) {return self.e720_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
fn guard_e726_c(&mut self,state:State)->Step {
if !self.can_replay(3) {return self.e726_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
fn guard_e733_c(&mut self,state:State)->Step {
if !self.can_replay(3) {return self.e733_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
fn guard_e809_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e809_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.diag_fail(t1.position::<false>(),"match");
...
fn guard_e810_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e810_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.diag_fail(t1.position::<false>(),"if");
...
fn guard_e815_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e815_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.diag_fail(t1.position::<false>(),"external");
...
if !self.can_replay(2) {return self.e819_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.diag_fail(t1.position::<false>(),"$");
...
if !self.can_replay(2) {return self.e820_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.diag_fail(t1.position::<false>(),"$");
...
if !self.can_replay(1) {return self.e823_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.diag_fail(t1.position::<false>(),"$");
...
fn guard_e824_c(&mut self,state:State)->Step {
if !self.can_replay(2) {return self.e824_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
fn guard_e866_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e866_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
...
fn guard_e867_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e867_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.diag_fail(t1.position::<false>(),"if");
...
if !self.can_replay(3) {return self.e871_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
fn guard_e872_c(&mut self,state:State)->Step {
if !self.can_replay(3) {return self.e872_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
if !self.can_replay(1) {return self.e874_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
...
if !self.can_replay(1) {return self.e875_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
...
fn guard_e882_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e882_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.diag_fail(t1.position::<false>(),"$");
...
fn guard_e883_c(&mut self,state:State)->Step {
if !self.can_replay(2) {return self.e883_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
if !self.can_replay(0) {return self.e884_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.diag_fail(t1.position::<false>(),"(");
...
fn guard_e894_c(&mut self,state:State)->Step {
if !self.can_replay(6) {return self.e894_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
fn guard_e916_c(&mut self,state:State)->Step {
if !self.can_replay(5) {return self.e916_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
fn guard_e919_c(&mut self,state:State)->Step {
if !self.can_replay(1) {return self.e919_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.diag_fail(t1.position::<false>(),"external");
...
if !self.can_replay(1) {return self.e920_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.diag_fail(t1.position::<false>(),"$");
...
if !self.can_replay(2) {return self.e921_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
fn guard_e935_c(&mut self,state:State)->Step {
if !self.can_replay(6) {return self.e935_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
fn guard_e938_c(&mut self,state:State)->Step {
if !self.can_replay(5) {return self.e938_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
if !self.can_replay(2) {return self.e942_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
fn guard_e1034_c(&mut self,state:State)->Step {
if !self.can_replay(5) {return self.e1034_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
if !self.can_replay(2) {return self.e1038_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let mut d2=Diag::NONE;
...
fn guard_e1039_c(&mut self,state:State)->Step {
if !self.can_replay(0) {return self.e1039_c(state);}
if !DIAG {return Step {ok:false,state,events:EventId(0),diag:Diag::NONE};}
self.display_reach(state.consumed.max(state.matched));
let t1=self.trivia_1::<false>(state.entered(true,true)).state;
let d2=self.diag_fail(t1.position::<false>(),"(");
