...
public class ParseContext implements 
  ...
  // TODO store successfully token's <position,tokens> map
	...
	public static class Snapshot {
		public final Deque<TransactionElement> tokenStack;
		public final Map<ChoiceInterface, Parser> chosenParserByChoice;
		public final Map<NonOrdered, Parsers> orderedParsersByNonOrdered;
		public final Map<Parser, Map<Name, Object>> scopeTreeMapByParser;
		public final Map<Name, Object> globalScopeTreeMap;
		public final List<ParseFrame> parseFrames;

		public Snapshot(ParseContext context) {
			this.tokenStack = new ArrayDeque<>();
			for (TransactionElement te : context.tokenStack) {
				this.tokenStack.addLast(te.copy());
			}
			this.chosenParserByChoice = new HashMap<>(context.chosenParserByChoice);
			this.orderedParsersByNonOrdered = new HashMap<>(context.orderedParsersByNonOrdered);
			this.scopeTreeMapByParser = new HashMap<>();
			for (Map.Entry<Parser, Map<Name, Object>> entry : context.scopeTreeMapByParser.entrySet()) {
				this.scopeTreeMapByParser.put(entry.getKey(), new HashMap<>(entry.getValue()));
			}
			this.globalScopeTreeMap = new HashMap<>(context.globalScopeTreeMap);
			this.parseFrames = new ArrayList<>(context.parseFrames);
		}
	}
	...
	public void pushSnapshot() {
		if (!history.isEmpty()) {
			Snapshot last = history.peek();
			int lastPos = last.tokenStack.peekLast().getParserCursor().getPosition(TokenKind.consumed).value();
			if (lastPos == this.getConsumedPosition().value()) {
				return;
			}
		}
	...
	public boolean popSnapshot() {
		...
		// To go BACK, we need to discard current and peek the previous one.
		history.pop(); 
		if (history.isEmpty()) {
			return false; 
		}
		restore(history.peek()); // Restore to the one before
		return true;
	}

	public void restore(Snapshot snapshot) {
		this.tokenStack.clear();
		for (TransactionElement te : snapshot.tokenStack) {
			this.tokenStack.addLast(te.copy());
		}
		this.chosenParserByChoice.clear();
		this.chosenParserByChoice.putAll(snapshot.chosenParserByChoice);
		this.orderedParsersByNonOrdered.clear();
		this.orderedParsersByNonOrdered.putAll(snapshot.orderedParsersByNonOrdered);
		this.scopeTreeMapByParser.clear();
		for (Map.Entry<Parser, Map<Name, Object>> entry : snapshot.scopeTreeMapByParser.entrySet()) {
			this.scopeTreeMapByParser.put(entry.getKey(), new HashMap<>(entry.getValue()));
		}
		this.globalScopeTreeMap.clear();
		this.globalScopeTreeMap.putAll(snapshot.globalScopeTreeMap);
		this.parseFrames.clear();
		this.parseFrames.addAll(snapshot.parseFrames);
	}

  public final Deque<ParseFrame> parseFrames = new ArrayDeque<ParseFrame>();
  public int farthestConsumedOffset = 0;
  ...
  List<ParseFailureDiagnostics.ExpectedHintCandidate> expectedHintCandidatesAtFarthestFailure =
	...
	public void onCommit(Parser parser, org.unlaxer.TokenList tokens) {
	  for (TransactionListener listener : listenerByName.values()) {
	    listener.onCommit(this, parser, tokens);
	  }
	}
