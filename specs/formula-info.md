...
- 区切り行は行頭の `---END_OF_PART---` で、その後ろには空白 `' '` とタブ `'\t'` だけを置ける（改行または入力末尾まで）。
- `---END_OF_PART---` で始まり、その後ろに空白・タブ以外の文字が続く行はエラー（`FormulaInfoParseException`）。値の続きの行としては扱わない（2.0.1、issue #211。以前は値の続きになり、次の式定義が黙って同じブロックに合流していた）。
- 1 つの式定義に `calculatorName` が 2 回以上あるとエラー（`FormulaInfoParseException`）。区切り行の書き忘れで 2 つの式定義が合流した印として扱う。他のキーの重複は従来どおり（`tags` は累積、それ以外は後勝ち）。
- 最後の式定義は区切り行なしで入力末尾に達してもよい。
