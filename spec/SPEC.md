...
graph TB
    ...
    subgraph LEGACY["レガシーパーサー (unlaxer-common)"]
        LP["手書きパーサーコンビネータ"]
        PT["ParseTree"]
        AST["AST"]
        LP --> PT --> AST
    ...
    subgraph P4STACK["P4 パーサー (UBNF 自動生成)"]
        P4P["TinyExpressionP4Parsers"]
        P4PT["P4 ParseTree"]
    ...
    style B5 fill:#d4f1d4,stroke:#2e7d32
...
sequenceDiagram
    ...
    MemCL-->>Compiler: Class<? extends PreConstructedCalculator>
    Compiler-->>DslCalc: compiled class
    DslCalc->>Calc: apply(context)
    Calc-->>DslCalc: result (Number / String / Boolean / Object)
...
classDiagram
    ...
    NumericTypes <|-- IntegralTypes
    NumericTypes <|-- FloatingTypes

    ExpressionTypes <.. NumericTypes : _byte, _short, _int, _long
    ExpressionTypes <.. IntegralTypes : _byte, _short, _int, _long
    ExpressionTypes <.. FloatingTypes : _float, _double
    ExpressionTypes <.. NumericTypes : bigDecimal, bigInteger

    class SpecifiedExpressionTypes {
        +ExpressionTypes evalType
        +ExpressionTypes resultType
        +SpecifiedExpressionTypes(ExpressionTypes, ExpressionTypes)
    }
...
- 区切り行の `---END_OF_PART---` の後ろには空白・タブだけを置ける。それ以外の文字が続く行と、1 ブロックに `calculatorName` が 2 回以上あるブロックはエラー（2.0.1、issue #211。詳細は `specs/formula-info.md`）
...
stateDiagram-v2
    ...
    Cached --> MemCLAlive : MemoryClassLoader 参照保持中
    MemCLAlive --> MemCLGCed : GC によるクラス破棄
    MemCLGCed --> Unloaded : コンパイル済みクラス消失

    note right of Cached
        List<Calculator> を TenantID キーで保持
        Calculator.apply() はステートレス・スレッドセーフ
    end note

    note right of Compiling
        JAVA_CODE バックエンド: javac 実行（高コスト）
        AST_EVALUATOR: パースのみ（低コスト）
    end note
```
